...
本手册针对 `qubit-fs` `0.2`、Rust 1.94 及以上版本。读者是通过已配置文件系统发布报告，
...
`qubit-fs = { version = "0.2", features = ["async"] }`，并使用自己已有的执行器；库不要求 Tokio。
...
临时文件和目录有独立的所有权生命周期，提供 `cleanup`、`keep` 和 `persist`。0.2 的
