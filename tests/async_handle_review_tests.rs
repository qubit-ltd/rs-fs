...
fn test_async_facade_rejects_invalid_stat_and_opened_identities() {
    ...
    let reader =
        ready(file_system.open_reader(&path("/expected"), qubit_fs::ReadOptions::default()))
...
fn test_async_facade_enriches_direct_provider_failures() {
    ...
    for error in [
        ...
        ready(file_system.create_directory(&target, qubit_fs::CreateDirectoryOptions::default()))
            .expect_err("create provider failure should propagate"),
        ready(file_system.delete_file(&target, qubit_fs::DeleteOptions::default()))
            .expect_err("delete-file provider failure should propagate"),
        ready(file_system.delete_directory(&target, qubit_fs::DeleteOptions::default()))
...
fn test_async_facade_enriches_handle_and_temp_provider_failures() {
    ...
    for stage in [AsyncCopyStage::OpenReader, AsyncCopyStage::OpenWriter] {
        ...
        let error = match stage {
            AsyncCopyStage::OpenReader => {
                ready(file_system.open_reader(&target, qubit_fs::ReadOptions::default()))
    ...
    let Err(file) = ready(file_system.create_temp_file(TempFileOptions::default())) else {
        panic!("temporary-file provider failure should propagate");
    };
    assert_eq!(FsErrorKind::UnsupportedOperation, file.kind());
    let Err(directory) = ready(file_system.create_temp_directory(TempDirectoryOptions::default()))
    else {
...
fn test_async_facade_dispatches_successful_operations() {
    ...
    let mut reader = ready(file_system.open_reader(&source, qubit_fs::ReadOptions::default()))
    ...
    assert_eq!(
        5,
        ready(reader.read_fully_async(&mut bytes)).expect("reader should return bytes")
    );
    ...
    ready(writer.write_fully_async(b"bytes")).expect("writer should accept all bytes");
    ...
    assert_eq!((source.clone(), target.clone()), {
        let outcome =
            ready(file_system.rename(&source, &target, qubit_fs::RenameOptions::default()))
...
fn test_async_facade_rejects_unrequested_idempotent_outcomes() {
    ...
    let delete_error = ready(file_system.delete_file(&target, qubit_fs::DeleteOptions::default()))
