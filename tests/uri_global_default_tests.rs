...
fn test_uri_credential_boundaries_ignore_global_allow_rules() {
    ...
    let connection =
        ConnectionUri::parse_with_policy("s3://user:raw-password@bucket/key?tenant_payload=raw-secret", &policy)
            .expect("connection URI should parse");
    let rendered = connection.to_string();
    assert!(!rendered.contains("raw-password"));
    assert!(!rendered.contains("raw-secret"));

    let redaction = Redactor::new(policy).redact_uri("s3://bucket/key?tenant_payload=raw-secret");
    assert_ne!("s3://bucket/key?tenant_payload=raw-secret", redaction.text().as_str(),);
