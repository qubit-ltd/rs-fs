...
fn test_temp_directory_child_components_are_lexically_safe() {
    ...
    let descendant = RelativePath::parse("nested/child").expect("relative path should parse");
    assert_eq!("/temporary/nested/child", directory.descendant(&descendant).as_str());
...
fn test_temp_directory_persist_marks_resource_persisted() {
    ...
    let mut directory = filesystem
        .create_temp_directory(TempOptions::default())
        .expect("temporary directory should open");
    let target = Path::parse("/published-directory").expect("target should parse");
...
fn test_temp_directory_persist_failure_preserves_provider_progress() {
    for (failure_state, expected_state) in [
        ...
        assert_eq!(
            Some(&Path::parse("/published-directory").expect("target should parse")),
