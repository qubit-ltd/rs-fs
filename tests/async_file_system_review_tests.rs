...
fn test_async_facade_rejects_contract_and_fallback_boundary_failures() {
    ...
    let Err(invalid_path) = ready(file_system.create_temp_file(TempFileOptions::default())) else {
...
fn test_async_facade_preflight_rejects_invalid_paths_options_and_capabilities() {
    ...
    let (file_system, _) = async_recording_file_system(AsyncRecordingConfig::default());
    let mut temporary = ready(file_system.create_temp_file(TempFileOptions::default()))
