...
use crate::common::poll_support::assert_pending;
...
impl AsyncFileSystemSpi for StreamCopyFallbackSpi {
    ...
    fn stat<'a>(&'a self, request: StatRequest<'a>) -> SpiFuture<'a, FsResult<StatResponse>> {
        if let Some(kind) = self.stat_error {
            return Box::pin(async move { Err(FsError::new(kind, FsOperation::Stat, "injected stat failure")) });
        }
...
fn test_async_facade_exists_contextualizes_other_errors() {
    let file_system = AsyncFileSystem::from_spi(StreamCopyFallbackSpi::new(false, Some(FsErrorKind::Io)))
        .expect("test SPI should construct");
    let error =
        ready(file_system.exists(&path("/io-error"))).expect_err("exists should contextualize non-`NotFound` failures");
...
fn test_async_facade_stream_copy_commit_already_exists_with_skip_marks_skipped() {
    let file_system =
        AsyncFileSystem::from_spi(StreamCopyFallbackSpi::new(true, None)).expect("test SPI should construct");
    ...
    let outcome = ready(operation.execute()).expect("commit refusal after stream write should skip");
...
fn test_async_facade_stat_and_open_pending_and_error() {
    for stage in [
        ...
        match stage {
            ...
            AsyncCopyStage::OpenReader => {
                assert_pending(Box::pin(fs.open_reader(&path("/file"), ReadOptions::default())).as_mut())
            }
            AsyncCopyStage::OpenWriter => {
                assert_pending(Box::pin(fs.open_writer(&path("/file"), WriteOptions::default())).as_mut())
            }
            _ => unreachable!(),
        }
        let (fs, _) = async_recording_file_system(AsyncRecordingConfig {
        ...
        let error = match stage {
            AsyncCopyStage::Stat => ready(fs.stat(&path("/file"))).expect_err("provider failure expected"),
            AsyncCopyStage::OpenReader => {
                let Err(error) = ready(fs.open_reader(&path("/file"), ReadOptions::default())) else {
...
fn test_async_rename_preflight_and_result_identity() {
    ...
    let outcome = ready(fs.rename(&source, &target, RenameOptions::default())).expect("rename should succeed");
    assert_eq!(&source, outcome.source());
    assert_eq!(&target, outcome.target());
    assert_eq!(vec!["rename"], probe.calls());
    let failure =
        ready(fs.rename(&source, &source, RenameOptions::default())).expect_err("same path must fail locally");
...
fn test_async_facade_convenience_operations_enforce_contracts() {
    ...
    .0;
    let read_error = ready(read_error_file_system.read_all(&path("/file"), ReadOptions::default(), 5))
...
fn test_async_facade_rejects_contract_and_fallback_boundary_failures() {
    ...
    let durability = ready(operation.execute()).expect_err("non-durable provider outcome must be rejected");
    assert_eq!(FsErrorKind::ProviderContractViolation, durability.error().kind());

    let (file_system, _) = async_recording_file_system(AsyncRecordingConfig::default());
    let Err(same) = file_system.begin_copy(source.clone(), source.clone(), CopyOptions::default()) else {
    ...
    for config in [
        ...
        let mut operation = file_system
            ...
            .expect("copy preflight should succeed");
        let failure = ready(operation.execute()).expect_err("declined fallback boundary failure must propagate");
...
fn test_async_facade_rejects_unsupported_capabilities_and_path_semantics() {
    ...
    let error =
        ready(file_system.open_reader(&source, ReadOptions::default())).expect_err("read capability must be required");
    ...
    let object_key =
        Path::parse_with_semantics("/provider-literal", PathSemantics::ObjectKey).expect("object key should parse");
    let error = ready(file_system.stat(&object_key)).expect_err("foreign path semantics must be rejected");
...
fn test_async_facade_preflight_rejects_invalid_paths_options_and_capabilities() {
    ...
    assert!(ready(read_limited.open_reader(&source, ReadOptions::default().with_length(Some(2)),),).is_err());
    ...
    assert!(ready(file_system.create_directory(&relative, CreateDirectoryOptions::default()),).is_err());
    ...
    assert!(ready(file_system.delete_file(&target, DeleteOptions::default().with_recursive(true),),).is_err());
    ...
    let (file_system, _) = async_recording_file_system(AsyncRecordingConfig::default());
    let mut temporary =
        ready(file_system.create_temp_file(TempOptions::default())).expect("temporary file should be created");
