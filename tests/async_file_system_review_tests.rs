...
fn test_async_write_all_commit_failure_retains_writer() {
    ...
    let mut operation = filesystem
        ...
        .expect("write preflight should succeed");
    let failure =
        ready(operation.execute()).expect_err("injected commit failure should retain writer");
...
fn test_async_facade_stat_and_open_pending_and_error() {
    for stage in [
        ...
        let error = match stage {
            ...
            AsyncCopyStage::OpenWriter => {
                let Err(failure) = ready(fs.open_writer(&path("/file"), WriteOptions::default()))
                else {
                    panic!("provider failure expected");
                };
...
fn test_async_writer_rejects_non_durable_required_commit_outcome() {
    let file_system = AsyncFileSystem::from_spi(StreamCopyFallbackSpi::new(false, None))
        .expect("test SPI should construct");
    ...
    let failure = ready(writer.commit_async())
...
fn test_async_facade_preflight_rejects_invalid_paths_options_and_capabilities() {
    ...
    assert!(
        ready(file_system.list(&ListScope::Path((relative).clone()), ListOptions::default()))
            .is_err()
    );
    ...
    assert!(
        ready(without_list.list(&ListScope::Path((source).clone()), ListOptions::default()))
            .is_err()
    );
    ...
    let (file_system, _) = async_recording_file_system(AsyncRecordingConfig::default());
    let mut temporary = ready(file_system.create_temp_file(TempOptions::default()))
