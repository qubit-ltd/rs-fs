...
impl ProviderProperties {
    ...
    fn missing_capability_operation(
        ...
        match capability {
            FileSystemCapability::List => (!operations
                .supports(ProviderOperation::List))
            .then_some(ProviderOperation::List),
            ...
            | FileSystemCapability::AtomicReplace => (!operations
                .supports(ProviderOperation::OpenWriter))
            .then_some(ProviderOperation::OpenWriter),
            FileSystemCapability::CreateDirectory
            | FileSystemCapability::EmptyDirectory => (!operations
                .supports(ProviderOperation::CreateDirectory))
...
mod tests {
    ...
    fn operations_without(
        missing: Option<ProviderOperation>,
    ) -> ProviderOperations {
    ...
    fn test_missing_capability_operation_maps_each_capability_directly() {
        ...
        for (capability, required_operation) in cases {
            let properties = properties_with_operations(operations_without(
                required_operation,
            ));
            assert_eq!(
                required_operation,
            ...
            match required_operation {
                Some(required_operation) => {
                    ...
                    assert!(
                        format!("{error}")
                            .contains(&format!("{required_operation:?}"))
                    );
                }
                None => assert!(
                    properties
