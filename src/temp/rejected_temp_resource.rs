...
// =============================================================================
//! Explicit cleanup authority for a rejected temp session.
...
impl RejectedTempResource {
    ...
    pub(crate) fn new(session: Box<dyn TempResourceSpi>, provider: &str, path: Option<Path>) -> Self {
        Self {
            session,
    ...
    pub fn cleanup(&mut self) -> FsResult<()> {
        ...
        let result = self.session.cleanup();
...
impl Debug for RejectedTempResource {
    /// Omits the session and unvalidated provider identity.
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        f.debug_struct("RejectedTempResource")
